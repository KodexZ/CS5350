...
def test_bank_data():
    train = pd.read_csv('C:/Users/korto/School/CS5350/TestingDataBank/train.csv')
    train = train.rename(columns={train.columns[-1] : 'P'})
    test = pd.read_csv('C:/Users/korto/School/CS5350/TestingDataBank/test.csv')
    test = test.rename(columns={test.columns[-1] : 'P'})
...
def create_bag(original : DataFrame, tot_features, wanted_features):
    res = original.sample(len(original), replace=True)
    lst = random.sample([i for i in range(0, tot_features)], tot_features - wanted_features)
    res = res.drop(res.columns[lst], 1)
    return res
    



def BagTreeError(csvpath : str, itrs : int, depth = 3):  
    ...
    trees = []
    for i in range(itrs):
        bag = create_bag(train, 16, 4)
        treehere = DecisionTree(bag, depth)
        trees.append(treehere)
        correcttrain = sum(row['P'] == np.sign(sum(tree.predict(row) for tree in trees)) for i, row in train.iterrows())
        trainerr = 1-correcttrain/len(train)
        correcttest = sum(row['P'] == np.sign(sum(tree.predict(row) for tree in trees)) for i, row in test.iterrows())
        testerr = 1 - correcttest/len(test)
        print(f'{trainerr} {testerr}')
...
if __name__ == '__main__':
    ...
    BagTreeError('C:/Users/korto/School/CS5350/TestingDataBank/', 100)
