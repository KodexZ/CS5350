...
def model_optimizer():
    params = dict()
    params['learning_rate'] = skopt.space.Real(.01, 1)
    params['n_estimators'] = skopt.space.Integer(100, 1000)
    params['max_depth'] = skopt.space.Integer(1, 7)
    params['colsample_bytree'] = skopt.space.Real(.5, 1)
    params['subsample'] = skopt.space.Real(.6, 1)
    search = BayesSearchCV(estimator=XGBClassifier(), search_spaces=params, n_jobs=-1, cv=StratifiedKFold(n_splits=5, shuffle=True))
...
gbc = XGBClassifier(colsample_bytree=.5, learning_rate=.20855, max_depth=4, n_estimators=100, subsample=1)
